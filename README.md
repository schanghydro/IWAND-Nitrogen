...
Water quantity predictions have made tremendous progress, from model development to gaining new knowledge through Large-Sample Hydrology (LSH), going beyond single-case studies and individual rivers. The backbone behind such progress is the availability of a ready-to-use benchmarks. Here, we developed the Integrated Watershed Attributes, and Nutrient Data for Nitrogen (IWAND-Nitrogen) dataset for the contiguous United States. The IWAND-Nitrogen dataset includes nearly 574,767 in-situ nitrogen (NO3) records from 1,877 different catchments, each with at least 200 measurements from 1980 to 2023. This dataset pairs 93 watershed attributes for both the total upstream area and local corridor, eight nitrogen input forcings (both basin-averaged and gridded), and eleven climate forcings. Our vision for IWAND is to serve as a nutrient data benchmark for the water quality community, facilitating accurate modelling, deeper physical understanding, and better adaptive strategies
...
- b) Directly use watershed boundary files from [GAGES-II](https://pubs.usgs.gov/publication/70046617) for gauges in Group I and III
- c) Directly use watershed boundary files from [ WaterTrends dataset](https://data.usgs.gov/datacatalog/data/USGS:57a9e239e4b05e859be05534) for gauges in Group II and IV
- d) Extract watershed boundary files using our developed automatic watershed extraction algorithm for gauges in Group V 
## (5) Forcing time-series extraction 
Time-series N input forings from gTREND-Nitrogen and climate forcings from [NLDAS2](https://ldas.gsfc.nasa.gov/nldas) (North American Land Data Assimilation System) were extracted based on watershed boundary. 

The climate forcing extraction was done using two ways: 
- a) **Scrips/Forcing_Extraction/nldas_point_extraction.py**: For watersheds smaller than NLDAS2 grid (~0.125 degree), the forcings were extracted from coordinates of the centroid of watersheds.
- b) **hydroData** (https://github.com/mhpi/hydroData): Used for the larger watersheds  
